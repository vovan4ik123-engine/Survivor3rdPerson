...
{
    BallGun::BallGun(float shotDelay) : BaseWeapon(shotDelay)
    {
        m_bullets.reserve(20);

        for(int i = 0; i < m_bullets.capacity(); ++i)
        {
            auto obj = std::make_shared<Beryll::SimpleCollidingObject>("models3D/player/PlayerBullet.fbx",
                                                                       m_bulletMass,
                                                                       true,
            ...
            obj->setOrigin(glm::vec3{0.0f, 1000.0f, 0.0f});
            obj->disableUpdate();
    ...
    void BallGun::draw(const glm::mat4& sunLightVPMatrix, const glm::vec3& sunLightDir, const std::shared_ptr<Beryll::Shader>& shader)
    {
        ...
        m_bulletTrajectory.calculateAndDraw(m_bulletMass,
    ...
    void BallGun::shoot()
    {
        if(m_lastShotTime + m_shotDelay < EnumsAndVars::mapPlayTimeSec)
        {
